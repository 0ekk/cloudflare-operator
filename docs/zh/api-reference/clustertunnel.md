...
ClusterTunnel 将 Tunnel 的功能扩展到集群级，允许多个命名空间使用同一个 Cloudflare Tunnel。这非常适合共享基础设施，单个隧道为不同命名空间中的应用程序提供服务。

### 主要特性

| 特性 | 描述 |
|------|------|
| **集群级访问** | 可从任何命名空间的 Ingress/Gateway 资源访问 |
| **资源共享** | 单个隧道服务多个命名空间 |
| **集中管理** | 在一个地方管理隧道配置 |
...
- **名称唯一性**: ClusterTunnel 名称在集群级作用域内必须唯一
...
- [Ingress 集成](../migration/tunnelbinding-migration.md#步骤-3创建-ingressclass) - 将 Ingress 与 ClusterTunnel 配合使用
- [Gateway API 集成](../migration/tunnelbinding-migration.md#迁移路径-2tunnelbinding-到-gateway-api) - 将 Gateway API 与 ClusterTunnel 配合使用
